...
simulation_study <- function(snr = 5, sample_size = c(100, 200, 500), N = 1000, num_resampling_CV = 100
  ...
  for(s in 1:length_sample_size){
    ...
    if(K0_est == 1){
        est_eta[l, 1] <- svd_Gamma$d[1]
        est_beta[l, 1] <- svd_Gamma$d[1] / (t(B_tilde %*% CoFA_CrossCov_fit$U) %*%  W_mat %*% cov_fpcaSC_cov_hat %*%  W_mat %*% B_tilde %*% CoFA_CrossCov_fit$U  )
        est_lambda0[l, 1] <-   (t(B_tilde %*% CoFA_CrossCov_fit$U) %*%  W_mat %*% cov_fpcaSC_cov_hat %*%  W_mat %*% B_tilde %*% CoFA_CrossCov_fit$U  )

    } else {
        for(j in 1:K0_est){
          est_eta[l, j] <- svd_Gamma$d[j]
